...
swapin(char* mem, pte_t *pte){ // 线性地址、页表项
  uint blockno = *pte >> 12; // 取磁盘号
  // cprintf("换进来的block number是：%p\n",blockno);
...
void
 pgfault()
 {
  //  cprintf("pafault  running -----!!!\n");
  unsigned addr = PGROUNDDOWN(rcr2()); // 获取导致中断的线性地址,要取整
  char* mem;
 
  if(addr > proc->sz) { // 越界
    cprintf("kalloc out of memory!  %p  %d\n",addr,proc->pid);
    proc->killed = 1;
    return;
  } 
 
  mem = kalloc(); // 分配一块物理页帧
  // cprintf("kalloc()的返回值%p\n",mem);
  while(mem == 0) { // 没有页帧了
    cprintf("执行换出操作\n");
    // bstat();
    // 上锁
  ...
  pte_t* pte = walkpgdir(proc->pgdir, (void*)addr, 1);//查看va的页表项
